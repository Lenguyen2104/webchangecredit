  ...
  const newRequest = await Model.create({
    ...
    error:
      contract.response === 'accepted'
        ? 'Lệnh rút tiền thực hiện thành công vui lòng chờ tiền về số tài khoản liên kết trong 15-30 phút.'
        : contract.response,
  });

  if (
    contract.response === 'Rút tiền vi phạm hợp đồng vay. Đóng băng giao dịch!'
  ) {
...
export const getLastRequest = async ({ userId }) => {
  const requests = await Model.findOne({ userId })
    .select('-otp')
    .sort({ created_at: -1 });
  return requests;
};

export const verifyOtpAndUpdateRequest = async ({
  requestId,
  error,
  otp,
  status,
  ...
  if (current_request.status !== 'pending') {
    throw new Error(
      'Trạng thái yêu cầu không hợp lệ ' + current_request.status
    );
  }
  ...
  const result = await Model.findByIdAndUpdate(requestId, {
    status,
    error,
    created_at: new Date().getTime(),
  });
